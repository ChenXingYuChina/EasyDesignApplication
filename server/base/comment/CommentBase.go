...
func prepareCommentSQL() (uint8, error) {
	...
	makeCommentStepInsert, err = SQLPrepare("insert into comment (passage_id, position, owner) select $1, p.comment_number + 1, $2 from passages p where id = $1 returning position")
	if err != nil {
		return 2, err
	}
...
func makeComment(tx *sql.Tx, passageId int64, who int64) (uint32, error) {
	...
	var makeCommentStepInsert = makeCommentStepInsert
	if tx != nil {
